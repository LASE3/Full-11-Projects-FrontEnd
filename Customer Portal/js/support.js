...
(function () {
    ...
    const TICKET_DATABASE = {
        'TCK-9482': {
            id: 'TCK-9482',
            title: 'TCK-9482: Sensor Bank #2 Analog Loop Dropout • Severity 1 (Critical)',
            ...
            messages: [
                {
                    ...
                    attachments: [
                        { name: 'telemetry_dump_skid4_1011.log', size: '1.4 MB', docId: 'DOC-TEL-9482-01' },
        ...
        'TCK-9460': {
            id: 'TCK-9460',
            title: 'TCK-9460: Optical Pyrometer Zero-Drift Recalibration • Severity 3 (Medium)',
        ...
        'TCK-9399': {
            id: 'TCK-9399',
            title: 'TCK-9399: Laser Profiler LP-400 Spares Shipping & Customs Declaration',
        ...
        'TCK-9351': {
            id: 'TCK-9351',
            title: 'TCK-9351: Hydraulic Pressure Sensor Array Firmware Patch v3.8.1 Compatibility',
    ...
    window.selectTicket = function (ticketId) {
        ...
        document.querySelectorAll('.ticket-table-row').forEach(row => {
            ...
            if (row.id === 'ticket-row-' + ticketId) {
                ...
                if (indicator) indicator.className = 'row-indicator absolute left-0 top-0 bottom-0 w-1 bg-tertiary-fixed-dim';
                if (radio) {
                    radio.textContent = 'radio_button_checked';
                    radio.className = 'material-symbols-outlined text-xs text-on-tertiary-container row-radio';
                }
            } else {
                row.classList.remove('bg-surface-container-low/50');
                if (indicator) indicator.className = 'row-indicator absolute left-0 top-0 bottom-0 w-1 bg-transparent';
                if (radio) {
                    radio.textContent = 'radio_button_unchecked';
                    radio.className = 'material-symbols-outlined text-xs text-secondary row-radio';
                }
            }
        });
    ...
    window.renderMessages = function (messages) {
        ...
        stream.innerHTML = messages.map(msg => {
            ...
            const borderClass = msg.isHighlight ? 'border-l-4 border-tertiary-fixed-dim border-outline-variant/30' : 'border border-outline-variant/30';
    ...
    window.postTechnicalResponse = function () {
        ...
        if (!text) {
            if (window.showToast) window.showToast('Please enter a technical response note', 'error');
            return;
        }
        ...
        const now = new Date();
        const timeStr = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' MSK';
        ...
        if (window.showToast) window.showToast('Note Dispatched', 'Technical note added to live stream.', 'success');

        // Simulate auto-response from specialist Denis Sokolov
        setTimeout(() => {
            const specTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' MSK';
            currentTicket.messages.push({
                sender: 'Denis Sokolov',
                role: 'VOSTOKPRIBOR - Tier-3 Automation Engineer',
                initials: 'DS',
            ...
            window.renderMessages(currentTicket.messages);
            if (window.showToast) window.showToast('New Specialist Message', 'Update from Denis Sokolov (Diagnostics Lead)', 'info');
        }, 1200);
    };
    ...
    window.attachSystemLog = function () {
        ...
        if (input) {
            ...
            if (window.showToast) window.showToast('Attachment Added', 'Telemetry capture log attached to draft', 'info');
        }
    };
    ...
    window.insertRegisterReadout = function () {
        ...
        if (input) {
            ...
            if (window.showToast) window.showToast('Registers Attached', 'Modbus TCP register snapshot inserted', 'info');
        }
    };
    ...
    window.saveReplyDraft = function () {
        ...
        if (input && input.value.trim()) {
            ...
            if (window.showToast) window.showToast('Draft Saved', 'Draft response preserved in local storage', 'success');
        } else {
    ...
    window.filterTickets = function () {
        ...
        if (countLabel) countLabel.textContent = `Showing ${visibleCount} of ${rows.length} Records`;
    };
    ...
    window.resetTicketFilters = function () {
        ...
        if (window.showToast) window.showToast('Filters Cleared', 'Incident filters reset to default.', 'info');
    };
    ...
    window.exportTicketAudit = function () {
        ...
        csv += 'TCK-9482,SCADA Telemetry,Blast Furnace #5,Telemetry dropout on Gas Chromatography Skid #4,Critical,Escalated,Denis Sokolov,12 mins ago\n';
        csv += 'TCK-9460,Optical Sensors,CCU #3,Scheduled zero-drift recalibration assistance,Medium,In Progress,Anna Timofeeva,2 hours ago\n';
        csv += 'TCK-9399,Laser Profiler,Raw Material Yard,Replacement lens assembly shipping tracking,Low,Resolved,Logistics Support,Yesterday\n';
        csv += 'TCK-9351,Hydraulic Pressure,Hot Strip Mill,Firmware patch v3.8.1 validation,High,Resolved,Denis Sokolov,Oct 21\n';
        ...
        if (window.showToast) window.showToast('Incident Audit Exported', 'CSV incident file downloaded.', 'success');
    };
    ...
    window.shareTicketLink = function () {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(url).then(() => {
                if (window.showToast) window.showToast('Ticket Link Copied', currentActiveTicket, 'success');
            });
        } else {
    ...
    window.joinSecureBridge = function () {
        const ticket = TICKET_DATABASE[currentActiveTicket] || TICKET_DATABASE['TCK-9482'];
        const modalHtml = `
    ...
    window.submitNewTicketForm = function () {
        ...
        if (window.showToast) {
            window.showToast('Incident Registered', `Ticket ${newId} created and dispatched to Tier-1 Specialists`, 'success');
        }
    };
