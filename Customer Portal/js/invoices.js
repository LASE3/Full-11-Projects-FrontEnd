...
(function () {
  ...
  window.copyWirePacket = function () {
    ...
    navigator.clipboard
      ...
      .then(() => {
        if (window.showToast) {
          window.showToast(
            "Banking Wire Packet Copied",
            "Cleared for Sberbank / VTB treasury transfer.",
            "info",
          );
        } else {
  ...
  window.exportLedger = function () {
    if (window.showToast) {
      window.showToast(
        "Corporate Ledger Exported",
        "Severstal_Settlement_Ledger_2024.xlsx generated.",
        "info",
      );
    }
  };
  ...
  window.filterInvoiceStatus = function (btn, status) {
    ...
    document.querySelectorAll(".invoice-filter-tab").forEach((b) => {
      b.className =
        "invoice-filter-tab px-unit-base py-1 rounded text-on-surface-variant hover:text-on-surface font-body-md transition-colors";
    });
    if (btn) {
      btn.className =
        "invoice-filter-tab px-unit-base py-1 rounded bg-surface-container-lowest text-on-surface font-headline-sm text-body-md font-semibold shadow-sm";
    }
  ...
  window.showPaymentModal = function (invoiceRef, customAmount) {
    ...
    const title = isSpecific
      ? `Authorize Disbursement: ${invoiceRef}`
      : "Disburse Outstanding Accounts Payable";
    const amountFormatted = customAmount
      ? "$" + customAmount.toLocaleString("en-US") + ".00 USD"
      : isSpecific
        ? "$114,200.00 USD"
        : "$248,600.00 USD";
  ...
  window.executePaymentRelease = function (invRef, amount) {
    ...
    if (window.showToast) {
      window.showToast(
        `Wire Transmitted`,
        `Electronic wire of ${amount} transmitted to PJSC Sberbank for ${invRef}`,
        "success",
      );
    }
    ...
    if (invRef.includes("8819")) {
      ...
      if (r) {
        ...
        if (badge) {
          badge.className =
            "inline-flex items-center gap-1 px-unit-sm py-0.5 rounded bg-secondary-container text-on-secondary-container font-technical-tag text-technical-tag font-semibold";
          badge.innerHTML =
            '<span class="w-1.5 h-1.5 rounded-full bg-secondary"></span> Wire Transmitted / In Clearing';
        }
      }
    }
