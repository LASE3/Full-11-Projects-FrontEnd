...
(function () {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      if (kpiProjects) kpiProjects.textContent = d.active_projects ?? '—';
      if (kpiInvoices) kpiInvoices.textContent = d.open_invoices  ?? '—';
      if (kpiTickets)  kpiTickets.textContent  = d.open_tickets   ?? '—';
      if (kpiContract) kpiContract.textContent = d.total_contract_value
        ? ui.currency(d.total_contract_value, d.currency || 'USD')
        : '—';
  ...
  async function loadProjects() {
    ...
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:28px;"><span class="vp-spinner"></span> Loading projects…</td></tr>';
    ...
    try {
      ...
      if (!rows.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:28px;opacity:.5;">No projects found for your account.</td></tr>';
        return;
      }

      const statusColor = {
        'Active': '#2ecc71', 'In Progress': '#3498db',
        'Completed': '#9b59b6', 'On Hold': '#f39c12', 'Delayed': '#e74c3c'
      };
      ...
      tbody.innerHTML = '<tr><td colspan="7" style="color:#e74c3c;padding:28px;">Failed to load projects.</td></tr>';
    }
  }
  ...
  async function loadInvoices() {
    ...
    try {
      ...
      if (!rows.length) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:28px;opacity:.5;">No invoices on record.</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = rows.map(inv => {
        const badge = badgeMap[inv.payment_status] || 'color:#aaa;background:rgba(170,170,170,.1)';
        return `
      ...
      tbody.innerHTML = '<tr><td colspan="6" style="color:#e74c3c;padding:28px;">Failed to load invoices.</td></tr>';
    }
  }
  ...
  async function loadTickets() {
    ...
    list.innerHTML = '<div style="padding:24px;text-align:center;"><span class="vp-spinner"></span> Loading tickets…</div>';
    ...
    try {
      ...
      if (!tickets.length) {
        list.innerHTML = '<div style="padding:24px;text-align:center;opacity:.5;">No support tickets found.</div>';
        return;
      }

      const priorityColor = { Critical: '#e74c3c', High: '#f39c12', Medium: '#3498db', Low: '#2ecc71' };
      const statusColor   = { Open: '#2ecc71', 'In Progress': '#3498db', Closed: '#9b59b6', Pending: '#f39c12' };
        ...
        return `
          <div class="ticket-card" style="border-left:3px solid ${pc};">
            <div class="ticket-header">
              <span style="font-family:monospace;font-size:12px;font-weight:700;">${escHtml(t.ticket_id)}</span>
            ...
            <div class="ticket-subject" style="font-weight:700;margin:6px 0 4px;">${escHtml(t.subject)}</div>
            <div class="ticket-desc" style="font-size:12px;opacity:.65;line-height:1.4;">${escHtml((t.description || '').substring(0, 120))}${(t.description || '').length > 120 ? '…' : ''}</div>
            <div class="ticket-meta" style="margin-top:10px;display:flex;gap:16px;font-size:11px;opacity:.55;">
              ...
              <span>Category: ${escHtml(t.category || '—')}</span>
      ...
      list.innerHTML = '<div style="padding:24px;color:#e74c3c;">Failed to load tickets.</div>';
    }
  }
  ...
  function initTicketForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!payload.subject || !payload.description) {
        ui.toast('Validation', 'Subject and description are required.', 'warning');
        return;
      }

      try {
        await ui.withLoading(btn, customer.submitTicket(payload));
        ui.toast('Ticket Submitted', 'Your support request has been created.', 'success');
        form.reset();
