...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
      ...
      signBtn.addEventListener("click", function () {
        ...
        if (statusHeader) {
          ...
          statusHeader.className =
            "font-telemetry-micro text-[10px] text-secondary font-bold";
        }
        ...
        if (sign2Card) {
          ...
          if (icon) {
            ...
            icon.className =
              "material-symbols-outlined text-secondary text-[16px]";
          }
    ...
    const patchBtn = Array.from(document.querySelectorAll("button")).find((b) =>
      b.textContent.includes("Dispatch Industrial Patch"),
    );
    if (patchBtn) {
      patchBtn.addEventListener("click", function () {
        ...
        this.disabled = true;
        this.innerHTML =
          '<span class="material-symbols-outlined text-[16px] animate-spin">sync</span><span>Deploying SCADA Hotfix...</span>';

        setTimeout(() => {
          this.innerHTML =
            '<span class="material-symbols-outlined text-[16px]">check_circle</span><span>Patch v4.9.1 Deployed</span>';
          this.className =
            "h-control-height-md w-full bg-secondary text-white font-body-compact text-body-compact font-semibold flex items-center justify-center gap-space-xs cursor-default";
