...
(function () {
  ...
  window.selectBridge = function (
    ...
    if (buffText)
      buffText.textContent = `${Math.floor(buffer * 81.92)} / 8,192 LINES (${buffer}%)`;
    ...
    if (badgeElem && barElem && stripElem) {
      if (buffer > 70) {
        ...
        badgeElem.className =
          "font-label-uppercase text-label-uppercase text-error font-bold";
        barElem.className =
          "bg-error h-full transition-all duration-500 animate-pulse";
        stripElem.className = "absolute left-0 top-0 bottom-0 w-1 bg-error";
      } else if (buffer > 40) {
        badgeElem.textContent = "BUFFER NOMINAL / MODERATE LOAD";
        badgeElem.className =
          "font-label-uppercase text-label-uppercase text-tertiary-container font-bold";
        barElem.className =
          "bg-tertiary-fixed-dim h-full transition-all duration-500";
        stripElem.className = "absolute left-0 top-0 bottom-0 w-1 bg-[#D9822B]";
      } else {
        badgeElem.textContent = "BUFFER OPTIMAL";
        badgeElem.className =
          "font-label-uppercase text-label-uppercase text-secondary font-bold";
    ...
    document
      .querySelectorAll(".bridge-card")
      .forEach((c) => c.classList.remove("active-bridge"));
    const activeCard = Array.from(
      document.querySelectorAll(".bridge-card"),
    ...
    if (logBox) {
      ...
      entry.className =
        "flex items-start gap-space-xs py-0.5 border-b border-white/5 font-mono text-[11px]";
      const now = new Date();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnIncreaseBuffer) {
      btnIncreaseBuffer.addEventListener("click", function () {
        ...
        if (strip)
          strip.className = "absolute left-0 top-0 bottom-0 w-1 bg-secondary";
        ...
        window.showToast(
          "BUFFER COMMITTED",
          "Allocated +4 MB RAM pool to selected ingestion bridge.",
          "success",
          "memory",
        );
      });
    ...
    if (btnForceFlush) {
      btnForceFlush.addEventListener("click", function () {
        ...
        if (logBox) {
          const entry = document.createElement("div");
          entry.className =
            "flex items-start gap-space-xs text-secondary font-bold py-0.5 border-b border-white/5 font-mono text-[11px]";
        ...
        window.showToast(
          "BUFFER PURGED",
          "Manual ring-buffer flush committed to persistent write-once store.",
          "info",
          "cleaning_services",
        );
      });
