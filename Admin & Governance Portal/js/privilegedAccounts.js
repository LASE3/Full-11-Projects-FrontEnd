...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnRequestCred && reqModal) {
      btnRequestCred.addEventListener("click", () =>
        reqModal.classList.remove("hidden"),
      );
    }
    if (closeModalReq && reqModal) {
      closeModalReq.addEventListener("click", () =>
        reqModal.classList.add("hidden"),
      );
    }
    if (cancelModalReq && reqModal) {
      cancelModalReq.addEventListener("click", () =>
        reqModal.classList.add("hidden"),
      );
    }

    if (btnCheckinAll && checkinModal) {
      btnCheckinAll.addEventListener("click", () =>
        checkinModal.classList.remove("hidden"),
      );
    }
    if (closeModalCheckin && checkinModal) {
      closeModalCheckin.addEventListener("click", () =>
        checkinModal.classList.add("hidden"),
      );
    }
    if (cancelModalCheckin && checkinModal) {
      cancelModalCheckin.addEventListener("click", () =>
        checkinModal.classList.add("hidden"),
      );
    }
    ...
    document.querySelectorAll(".tier-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.querySelectorAll(".tier-btn").forEach((b) => {
          ...
          if (title)
            title.className =
              "font-security-stamp text-[11px] font-bold text-primary";
        });
        ...
        if (tier === "0") {
          ...
          btn.querySelector("span:first-child").className =
            "font-security-stamp text-[11px] font-bold text-error";
          if (notice) notice.classList.remove("hidden");
        } else if (tier === "1") {
          btn.classList.add("border-[#D9822B]", "bg-tertiary-fixed/20");
          btn.querySelector("span:first-child").className =
            "font-security-stamp text-[11px] font-bold text-[#D9822B]";
          if (notice) notice.classList.add("hidden");
        } else {
          btn.classList.add("border-secondary", "bg-secondary-container/20");
          btn.querySelector("span:first-child").className =
            "font-security-stamp text-[11px] font-bold text-secondary";
          if (notice) notice.classList.add("hidden");
        }
      });
    });
    ...
    document.querySelectorAll(".ttl-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.querySelectorAll(".ttl-btn").forEach((b) => {
          b.className =
            "ttl-btn py-space-xs px-1 text-center font-telemetry-micro text-[11px] rounded border border-outline-variant/50 hover:bg-surface-container font-medium";
        });
        btn.className =
          "ttl-btn py-space-xs px-1 text-center font-telemetry-micro text-[11px] rounded border-2 border-primary bg-primary-container text-on-primary font-bold";
    ...
    if (submitModalReq) {
      submitModalReq.addEventListener("click", () => {
        ...
        const asset = targetAssetEl
          ? targetAssetEl.value.split(" ")[0]
          : "SYS-11";
        ...
        newRow.className =
          "bg-secondary-container/20 border-l-4 border-l-secondary-fixed transition-colors vault-row";
        newRow.innerHTML = `
      ...
      confirmMassCheckin.addEventListener("click", () => {
        ...
        if (tbody) {
          tbody.querySelectorAll("tr").forEach((tr) => {
            const tag = tr.querySelector(
              ".bg-secondary-container\\/30, .bg-primary-fixed",
            );
            if (tag) {
              tag.className =
                "inline-flex items-center gap-[4px] px-space-xs py-[2px] bg-surface-container text-on-surface-variant font-security-stamp text-[10px] font-bold rounded";
              tag.innerHTML =
                '<span class="w-1.5 h-1.5 rounded-full bg-outline"></span>ROTATED &amp; VAULTED';
            }
          });
          tbody
            .querySelectorAll(
              ".bg-secondary-fixed, .animate-pulse, .animate-ping",
            )
            .forEach((el) => {
              el.classList.remove(
                "animate-pulse",
