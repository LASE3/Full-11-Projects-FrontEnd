...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const simBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Simulate Enforcement'));
        if (simBtn) {
        ...
        const exportBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Export Matrix'));
        if (exportBtn) {
            exportBtn.addEventListener('click', function () {
                ...
                const blob = new Blob([JSON.stringify(matrixExport, null, 2)], { type: 'application/json' });
                ...
                window.showToast('EXPORT DISPATCHED', 'Downloaded DOC-2026-007 schema ledger as verified cryptographic JSON package.', 'success', 'download');
            });
        }

        // 3. Commit Ledger Attestation
        const commitBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Commit Ledger Attestation'));
        if (commitBtn) {
        ...
        tierCards.forEach((card, idx) => {
            ...
            card.addEventListener('click', () => {
                ...
                roleRows.forEach(row => {
                    ...
                    if (!term || text.includes(term) || text.includes('root') || text.includes('clearance')) {
                        row.style.display = '';
                    } else {
                ...
                window.showToast('FILTER APPLIED', `Showing roles matching tier ${term.toUpperCase()}`, 'info', 'filter_alt');
            });
        });
    });
})();
