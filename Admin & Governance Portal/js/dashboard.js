...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (reCertDrawerOverlay) reCertDrawerOverlay.addEventListener('click', closeReCertDrawer);
        ...
        if (windowStatusToggle) {
            windowStatusToggle.addEventListener('click', function () {
                ...
                if (isWindowActive) {
                    ...
                    if (drawerStatusSub) drawerStatusSub.textContent = '30-Day Mandatory Review in progress';
                } else {
        ...
        tableRows.forEach(row => {
            row.addEventListener('click', function () {
                ...
                if (severBtn) {
                    ...
                    severBtn.className = 'w-full h-11 bg-error hover:bg-on-error-container text-on-error font-title-sm text-[13px] font-bold tracking-wider flex items-center justify-center gap-space-xs transition-colors shadow';
                    severBtn.innerHTML = '<span class="material-symbols-outlined text-[18px]">lock_reset</span><span>SEVER ALL ENTITLEMENTS</span>';
                }
            });
        });
        ...
        if (severBtn) {
            severBtn.addEventListener('click', function () {
                ...
                severBtn.innerHTML = '<span class="material-symbols-outlined text-[18px] animate-spin">refresh</span><span>EXECUTING REVOCATION ACROSS SYS 01-11...</span>';
                severBtn.classList.add('opacity-80');

                const targetEmp = drawerEmpId ? drawerEmpId.textContent : 'Target Account';

                setTimeout(() => {
                    severBtn.innerHTML = '<span class="material-symbols-outlined text-[18px]">check_circle</span><span>CREDENTIALS SEVERED &amp; PURGED</span>';
                    severBtn.className = 'w-full h-11 bg-inverse-surface text-inverse-on-surface font-title-sm text-[13px] font-bold tracking-wider flex items-center justify-center gap-space-xs transition-colors shadow cursor-default';

                    // Update corresponding row in table
                    const activeSelected = document.querySelector('.matrix-row.selected-row');
                    if (activeSelected) {
                        const statusBadge = activeSelected.querySelector('.status-badge');
                        if (statusBadge) {
                            statusBadge.className = 'px-space-xs py-[2px] bg-surface-container text-on-surface-variant font-security-stamp text-[10px] font-bold rounded';
                            statusBadge.textContent = 'REVOKED';
                        }
                    }

                    // Decrement Orphaned count
                    const orphanCard = document.querySelector('.text-headline-md.text-error');
