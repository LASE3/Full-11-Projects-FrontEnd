...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (streamBtn) {
            streamBtn.addEventListener('click', function () {
                ...
                if (streamPaused) {
                    ...
                    window.showToast('STREAM SUSPENDED', 'Event ingestion paused. Holding live frames in FIFO ring buffer.', 'warn', 'pause');
                } else {
                    ...
                    window.showToast('STREAM ACTIVE', 'Real-time telemetry stream resumed from 10 ingestion bridges.', 'success', 'play_arrow');
                }
            });
        }
        ...
        function executeLogFilter() {
            ...
            if (q !== '') {
                window.showToast('QUERY FILTERED', `Found ${matchCount} matching audit events for "${q}".`, 'info', 'search');
            }
        }
        ...
        setInterval(() => {
            ...
            row.className = 'flex items-center justify-between py-1 px-2 border-b border-white/5 font-mono text-[12px] log-entry-row';
            row.innerHTML = `
        ...
        // 4. Export Log Action
        const exportBtn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Export') || b.textContent.includes('Save Log'));
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                ...
                window.showToast('AUDIT LOG EXPORTED', 'Exported cryptographic terminal stream buffer.', 'success', 'download');
            });
        }
    });
})();
