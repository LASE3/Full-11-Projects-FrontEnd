    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (btnAutofill) {
            btnAutofill.addEventListener('click', () => {
                ...
                document.getElementById('reg-public-key').value = 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBmQ8eBaltNordScadaGatewayKey2026';
                ...
                document.querySelectorAll('.scope-card-option').forEach(card => {
                    ...
                    if (chk.value === 'telemetry:read' || chk.value === 'orders:read_write') {
                        chk.checked = true;
                        card.classList.add('selected');
                    }
                });

                window.showToast('PRESET LOADED', 'Loaded authoritative partner baseline for BaltNord Process Systems (CUS-1002).', 'info', 'dataset');
            });
        }
        ...
        if (regForm) {
            regForm.addEventListener('submit', (e) => {
                ...
                submitBtn.innerHTML = '<span class="material-symbols-outlined text-[16px] animate-spin">sync</span> Validating Credentials...';

                setTimeout(() => {
                    regForm.style.display = 'none';
                    if (regSuccessBox) {
                        regSuccessBox.style.display = 'block';
                        document.getElementById('disp-reg-ticket').innerText = 'ENCLAVE-REQ-' + Math.floor(100000 + Math.random() * 900000);
                    ...
                    window.showToast('REGISTRATION SUBMITTED', 'Clearance request routed to Jonas Richter (EMP-1020) & Dana Yermak (EMP-1017).', 'success', 'verified_user');
                }, 1000);
            });
        }
    });
