...
(function () {
    ...
    function initSearchAndFilter() {
        ...
        function filterRows() {
            ...
            rows.forEach(row => {
                ...
                const matchesQuery = !query || rowText.includes(query) || docId.includes(query) || docName.includes(query);
    ...
    function initFolderTree() {
        ...
        folderItems.forEach(item => {
            item.addEventListener('click', function () {
                ...
                window.showToast('FOLDER SELECTED', `Browsing repository partition: ${folderName}`, 'info', 'folder_open');
            });
        });
    }
    ...
    function initDocumentDrawer() {
        ...
        function openDrawerWithData(row) {
            ...
            if (downloadBtn) {
                downloadBtn.onclick = () => {
                    window.showToast('SECURE DECRYPTION IN PROGRESS', `Retrieving ${docName} from encrypted Almaty vault...`, 'info', 'lock_open');
                    setTimeout(() => {
                        window.showToast('DOWNLOAD READY', `${docName} integrity verified (SHA-256 OK). Transferred via TLS 1.3.`, 'success', 'download_done');
                    }, 1000);
                };
            }
