...
(function () {
    ...
    function initApprovalActions() {
        ...
        if (formSign) {
            formSign.addEventListener('submit', (e) => {
                ...
                submitBtn.innerHTML = '<span class="material-symbols-outlined text-[16px] animate-spin">sync</span> Affixing HSM Signature...';
                ...
                setTimeout(() => {
                    ...
                    if (step2) {
                        ...
                        step2.querySelector('.step-circle').innerHTML = '<span class="material-symbols-outlined text-[18px]">done</span>';
                    }
                    if (step3) {
                    ...
                    if (btnOpenSignModal) {
                        ...
                        btnOpenSignModal.innerHTML = '<span class="material-symbols-outlined text-[16px]">verified</span> Signed by Farida Iskakova (EMP-1019)';
    ...
    function initRedactionToggle() {
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            if (isRedacted) {
                toggleBtn.innerHTML = '<span class="material-symbols-outlined text-[16px]">visibility</span> View Unredacted (Internal Only)';
                document.querySelectorAll('.redact-target').forEach(el => {
                    el.classList.add('redacted-bar');
                });
                window.showToast('CUSTOMER VIEW APPLIED', 'Masked proprietary PLC memory maps and internal Almaty IP topology.', 'info', 'lock');
            } else {
                toggleBtn.innerHTML = '<span class="material-symbols-outlined text-[16px]">visibility_off</span> Preview Redacted (Customer Portal)';
                document.querySelectorAll('.redact-target').forEach(el => {
                    el.classList.remove('redacted-bar');
                });
                window.showToast('INTERNAL SPEC VIEW', 'Displaying full engineering specification with raw memory registers.', 'info', 'visibility');
            }
        });
    }
})();
