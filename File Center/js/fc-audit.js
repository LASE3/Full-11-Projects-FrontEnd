...
(function () {
    ...
    function initHashVerifier() {
        ...
        btnVerify.addEventListener('click', () => {
            ...
            if (!hash) {
                window.showToast('INPUT REQUIRED', 'Please paste a SHA-256 hash or select a document from the register.', 'error', 'error');
                return;
            }

            btnVerify.disabled = true;
            btnVerify.innerHTML = '<span class="material-symbols-outlined text-[14px] animate-spin">sync</span> Querying Hardware HSM Ledger...';

            setTimeout(() => {
                btnVerify.disabled = false;
                btnVerify.innerHTML = '<span class="material-symbols-outlined text-[14px]">security</span> Verify Cryptographic Seal';

                if (resultBox) {
                    resultBox.style.display = 'block';
                    document.getElementById('verify-disp-hash').textContent = hash;
                    document.getElementById('verify-disp-timestamp').textContent = new Date().toISOString() + ' (UTC+6)';
                }

                window.showToast('INTEGRITY CONFIRMED', 'Bitwise match verified against Almaty central hardware HSM enclave. Zero tampering detected.', 'success', 'verified');
            }, 800);
        });
    }
