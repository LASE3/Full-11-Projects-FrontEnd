...
(function () {
    ...
    function initAutofillPreset() {
        ...
        btnPreset.addEventListener('click', () => {
            document.getElementById('meta-doc-title').value = 'PRJ-2026-002_SCADA_Validation_Report.pdf';
            document.getElementById('meta-department').value = 'ENG';
            document.getElementById('meta-project-ref').value = 'PRJ-2026-002 (BaltNord Process Systems)';
            document.getElementById('meta-customer-ref').value = 'CUS-1002 (BaltNord)';
            document.getElementById('meta-retention').value = '7y';
            document.getElementById('meta-custodian').value = 'Farida Iskakova (EMP-1019)';
            document.getElementById('meta-description').value = 'Official automated integration and pressure telemetry test log for BaltNord SCADA interface.';
            ...
            if (fileNameElem) {
                fileNameElem.textContent = 'Selected: PRJ-2026-002_SCADA_Validation_Report.pdf (2.4 MB)';
                fileNameElem.style.display = 'block';
            }

            window.showToast('PRESET LOADED', 'Applied authoritative metadata for BaltNord PRJ-2026-002 test dossier.', 'info', 'dataset');
        });
    }
    ...
    function initUploadForm() {
        ...
        form.addEventListener('submit', (e) => {
            ...
            submitBtn.innerHTML = '<span class="material-symbols-outlined text-[16px] animate-spin">sync</span> Ingesting & Calculating Hash...';

            setTimeout(() => {
                form.style.display = 'none';
                if (successBox) {
                    successBox.style.display = 'block';
                    document.getElementById('disp-new-doc-id').textContent = 'DOC-2026-016';
                    document.getElementById('disp-new-doc-title').textContent = title;
                    document.getElementById('disp-new-doc-dept').textContent = dept;
                    document.getElementById('disp-new-doc-class').textContent = classification.toUpperCase();
                    document.getElementById('disp-new-doc-hash').textContent = '9a3f2b4c810d7e5e6c1a89b034298fc1c149afbf4c8996fb92427ae41e4649b9';
                }
