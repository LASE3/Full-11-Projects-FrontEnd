...
(function () {
  ...
  const POLICIES_DATA = [
    ...
    },
    {
      id: "DOC-2026-013",
      title:
        "Industrial Customer Support Service Level Agreement (SLA Tier 1-3)",
      category: "Operations",
      categoryKey: "templates",
      classification: "Internal",
      classCode: "internal",
  ...
  const CAROUSEL_SLIDES = [
    ...
    },
    {
      ...
      title:
        "VP-900 Ultra-Precise Industrial Flow Transmitter Receives CE Mark",
  ...
  function initCommandPalette() {
    ...
    function renderPaletteResults(query) {
      ...
      // Search employees
      const matchingEmployees = EMPLOYEES_DATA.filter(
        (emp) =>
      ...
      // Search policies
      const matchingPolicies = POLICIES_DATA.filter(
        (doc) =>
  ...
  function initHomeFeed() {
    ...
    function prevSlide() {
      currentSlideIdx =
        (currentSlideIdx - 1 + CAROUSEL_SLIDES.length) % CAROUSEL_SLIDES.length;
      renderSlide(currentSlideIdx);
    }
  ...
  function initEmployeeDirectory() {
    ...
    function filterAndRender() {
      ...
      const filtered = EMPLOYEES_DATA.filter((emp) => {
        ...
        const matchesClearance =
          clearance === "ALL" || emp.clearanceClass === clearance;
    ...
    if (clearanceSelect)
      clearanceSelect.addEventListener("change", filterAndRender);
    ...
    function filterAndRender() {
      ...
      const filtered = POLICIES_DATA.filter((doc) => {
        const matchesCategory =
          activeCategory === "all" || doc.categoryKey === activeCategory;
        const matchesQuery =
          !query ||
  ...
  window.switchHelpdeskTab = function (tabName) {
    ...
    if (btnHistory)
      btnHistory.classList.toggle("active", tabName === "history");
    ...
    if (statusPane)
      statusPane.style.display = tabName === "status" ? "block" : "none";
  };
  ...
  window.selectItCategory = function (el, cat) {
    ...
    if (container) {
      container
        .querySelectorAll(".it-category-chip")
